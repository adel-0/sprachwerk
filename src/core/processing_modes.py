...
class ProcessingMode(ABC):
    ...
    def _save_audio_recording(self, audio_data, filename_base, audio_capture):
        """Save audio recording with appropriate format based on capture type"""
    ...
    def _process_audio_data(self, audio_data, audio_file_path=None):
        """Process audio data through transcription, diarization, and alignment"""
        try:
            # Use provided audio file or process audio data directly
            if audio_file_path:
                ...
                # For in-memory audio data processing
            ...
            if audio_file_path:
                diarization_result = self.diarizer.diarize_file(audio_file_path)
            else:
                diarization_result = self.diarizer.diarize_chunk(audio_data)
    ...
    def _display_results(self, results):
        """Display processing results with metadata"""
    ...
    def run(self, *args, **kwargs):
        """Run the processing mode - to be implemented by subclasses"""
        pass


class BatchProcessingMode(ProcessingMode):
    """Batch processing mode implementation"""
    ...
    def _record_and_process(self, audio_capture, system_audio_capture, duration):
        ...
        try:
            ...
            if audio_data is None or (hasattr(audio_data, 'size') and audio_data.size == 0) or (hasattr(audio_data, '__len__') and len(audio_data) == 0):
...
class RealTimeProcessingMode(ProcessingMode):
    """Real-time processing mode implementation"""
    ...
    def _initialize_session(self):
        """Initialize session file for real-time recording"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
