...
class InteractiveMenu:
    ...
    def run(self):
        """Main menu loop with quick start options"""
        self._show_welcome()
        
        # Set mode to user's preferred mode if available
        preferred_mode = CONFIG.get('preferred_mode', 'realtime')
        if preferred_mode in ['realtime', 'batch']:
        ...
        while True:
            try:
                ...
                if choice is None:  # User cancelled
                    ...
                    # Start real-time
                    set_mode('realtime')
                    save_user_setting('preferred_mode', 'realtime')
                    ...
                    # Start batch
                    set_mode('batch')
                    save_user_setting('preferred_mode', 'batch')
                    save_user_settings_to_file()
    ...
    def _show_quick_start_menu(self):
        """Display the main start menu options"""
        print(f"\n{Fore.CYAN}{'='*70}{Style.RESET_ALL}")
    ...
    def _display_current_settings(self):
        ...
        # Current mode - get from CONFIG's preferred mode
        preferred_mode = CONFIG.get('preferred_mode', 'realtime')
        current_mode = "Real-time" if preferred_mode == 'realtime' else "Batch"
        ...
        print(f"  🎬 Mode: {Fore.GREEN}{current_mode}{Style.RESET_ALL}")
    ...
    def _get_recording_duration(self):
        """Get recording duration for batch mode"""
        ...
        while True:
            try:
                ...
                if choice is None:  # User cancelled
                    ...
                    duration_input = input(f"{Fore.YELLOW}Enter duration in seconds:{Style.RESET_ALL} ").strip()
                    try:
    ...
    def _get_audio_file_path(self):
        ...
        while True:
            try:
                file_path = input(f"{Fore.CYAN}Enter audio file path (or 'back' to return):{Style.RESET_ALL} ").strip()
    ...
    def _speaker_count_menu(self):
        """Speaker count configuration menu"""
        ...
        while True:
            ...
            try:
                ...
                if choice is None:  # User cancelled
                    ...
                    from src.core.config import save_user_setting, save_user_settings_to_file
                    save_user_setting('audio_device_index', None)
                    save_user_settings_to_file()
                    print(f"{Fore.GREEN}✓ Auto-detect mode enabled{Style.RESET_ALL}")
                    break
                elif choice == '2':
    ...
    def _test_audio_device(self, input_devices):
        """Test audio input from selected devices"""
        ...
        while True:
            try:
                device_input = input(f"{Fore.CYAN}Enter device number (or Enter for current):{Style.RESET_ALL} ").strip()
                
                if device_input == '':
                    # Test current device
                    device_idx = CONFIG.get('audio_device_index')
                    if device_idx is None:
                        print(f"{Fore.YELLOW}Testing auto-detect mode...{Style.RESET_ALL}")
                    else:
    ...
    def _audio_quality_menu(self):
        ...
        print(f"     {Fore.WHITE}   • Fastest processing, exact microphone capture{Style.RESET_ALL}")
        print(f"     {Fore.WHITE}   • May include background noise and artifacts{Style.RESET_ALL}")
        ...
        while True:
            try:
                ...
                if choice is None:  # User cancelled
                    ...
                    # Pre-processing (minimal processing)
    ...
    def _show_start_confirmation(self):
        ...
        mode = CONFIG.get('preferred_mode', 'realtime')
        if mode == 'realtime':
            print(f"\n{Fore.YELLOW}What will happen:{Style.RESET_ALL}")
            print(f"  • Real-time transcription will start")
            print(f"  • Audio will be captured and processed continuously")
            print(f"  • Transcription will appear in real-time")
            print(f"  • Press Ctrl+C to stop transcription")
        else:
            print(f"\n{Fore.YELLOW}What will happen:{Style.RESET_ALL}")
            print(f"  • Batch recording will start")
            print(f"  • Audio will be recorded for the specified duration")
            print(f"  • Processing will begin after recording completes")
            print(f"  • Results will be saved to the outputs folder")
    ...
    def _comprehensive_settings_menu(self):
        ...
        while True:
            ...
            print(f"  3. {Fore.CYAN}🎤 Audio Device Selection{Style.RESET_ALL}")
            print(f"     {Fore.WHITE}   • Choose microphone/input device{Style.RESET_ALL}")
            print(f"  4. {Fore.MAGENTA}🎵 Audio Quality Settings{Style.RESET_ALL}")
            print(f"     {Fore.WHITE}   • Configure audio processing{Style.RESET_ALL}")
            print(f"  5. {Fore.BLUE}🔊 Audio Source Selection{Style.RESET_ALL}")
